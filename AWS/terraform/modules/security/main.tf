...
# This module creates IAM roles, KMS keys, and security components
...
resource "aws_iam_role" "fargate_task" {
  name = "${var.project_name}-${var.environment}-fargate-task-role"
...
resource "aws_iam_role_policy" "fargate_task" {
  name = "${var.project_name}-${var.environment}-fargate-task-policy"
...
resource "aws_iam_role" "quicksight_execution" {
  name = "${var.project_name}-${var.environment}-quicksight-execution-role"
...
resource "aws_iam_role_policy" "quicksight_execution" {
  name = "${var.project_name}-${var.environment}-quicksight-execution-policy"
  role = aws_iam_role.quicksight_execution.id
...
resource "aws_iam_role_policy" "api_gateway_execution" {
  name = "${var.project_name}-${var.environment}-api-gateway-execution-policy"
...
resource "aws_iam_group_membership" "admin" {
  ...
  users = [aws_iam_user.admin[count.index].name]
  group = aws_iam_group.admin.name
}
